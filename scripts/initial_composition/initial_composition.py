...
class Nomoto2013Yields:
    ...
    def __init__(self, file_path: str, mass: int | float | str):
        ...
        if len(matching_cols) > 1:
            print(f"Warning: multiple columns match '{mass_str}': {matching_cols}. Using the first one.")   
        elif len(matching_cols) == 0:
            raise ValueError(f"Error: no columns match '{mass_str}' in dataframe columns {df.columns.tolist()}")
...
class InitialComposition:
    ...
    composition: dict
    def __init__(
        self, data_source: str | dict[str, float],
        ...
        if self.composition is not None:
            ...
            print("Warning: composition is None; X, Y, Z not computed.")
    ...
    def _from_file(self, path: str):
        ...
        comp = {
            n+str(mass_number[i]): mass_fraction[i] for i, n in enumerate(name)
        }
    ...
    def check_normalization(self):
        ...
        if abs(total_raw - 1.0) > self.norm_error:
            print(f"Warning: raw isotope sum = {total_raw:.6e} (expected ~1). Normalizing.")
    ...
    def save_to_file(self, path: str, ref_abd_path: str):
        ...
        comp = {iso_name: float(self.composition.get(iso_name, 0.0)) for iso_name in ref_abd.composition.keys()}
